...
def get_model_1(learning, file_weights):
    ...
    model.load_weights(file_weights, by_name=True)

    model.add(Flatten())
    model.add(Dense(7, kernel_initializer=glorot_uniform(seed), activation='softmax', name="new_dense"))
...
def train_networks(exp_name, model_type, learning_rate, training_size, batch_size, num_epochs, dataset_size, dataset_type,
    data_augmentation, weights_file):
    model = None

    if model_type == 1:
        model = get_model_1(learning_rate, weights_file)
    else:
        raise NotImplementedError
...
parser.add_argument('-w', dest='weights', type=str)
